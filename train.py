...
def split_dataset(test_ratio=0.2):
    dataset_size=len(os.listdir(os.path.join('train_data', 'gt_image')))
...
class Train:
    ...
    def _train(self):
        ...
        for e_p in range(self.epoch):
            etotal_loss = 0
            esegmentation_loss = 0
            ediscriminative_loss = 0
            cnt = 0
            for batch_data in self.data['train']:
                ...
                # print(input_data.shape, seg_mask.shape, instance_mask.shape)
                ...
                # if total_loss == np.nan or segmentation_loss == np.nan or discriminative_loss == np.nan:
                #     print(batch_data)
                total_loss1 = total_loss.detach().cpu().numpy()
                segmentation_loss1 = segmentation_loss.detach().cpu().numpy()
                discriminative_loss1 = discriminative_loss.detach().cpu().numpy()
                # if np.isnan(total_loss1) or np.isnan(segmentation_loss1) or np.isnan(discriminative_loss1):
                #     print(cnt)
                ...
                e=time.time()
                if cnt % 20 == 0:
                    print("total_loss:{:.6f}, seg_loss:{:.6f}, dis_loss:{:.6f}".format(total_loss,segmentation_loss,discriminative_loss))
                    log.write('time:{}, Total Loss:{}, Segmentation Loss:{}, Discriminative Loss:{}\n'.format(e-s,
                                                                                                               total_loss,
                                                                                                               segmentation_loss,
                                                                                                               discriminative_loss))
                    log.flush()
                etotal_loss += segmentation_loss + discriminative_loss
                esegmentation_loss += segmentation_loss
                ediscriminative_loss += discriminative_loss
            print(e_p, " total_loss:{:.6f}, seg_loss:{:.6f}, dis_loss:{:.6f}\n".format(etotal_loss, esegmentation_loss,
                                                                            ediscriminative_loss))
            log.write('Epoch:{}, Total Loss:{}, Segmentation Loss:{}, Discriminative Loss:{}\n'.format(e_p,
                                                                                                       etotal_loss,
                                                                                                       esegmentation_loss,
                                                                                                       ediscriminative_loss))
            log.flush()
            torch.save(self.model.state_dict(),os.path.join('./logs/models','model_1_{}_{}_{}.pkl'.format(start_time,e_p,etotal_loss)))
        log.close()
...
if __name__=='__main__':
    ...
    ap.add_argument('-t', '--test_ratio', default=0)#percent of train_data to be used in testing
    ap.add_argument('-ct', '--continue_train', default='no')#whether the current training loop is a continuation of a previous one
    ap.add_argument('-s', '--save', default='./logs/models/model_1_1648481010_326_2.7336108684539795.pkl')#location of the saved model checkpoint file

    args = vars(ap.parse_args())
    train_indices, test_indices = split_dataset(args['test_ratio'])
    data = build_sampler(TusimpleData('train_data'), args['batch'], 1, train_indices, test_indices)
    model = LaneNet()
