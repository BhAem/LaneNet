...
class Losses:
    ...
    def _bi_weight(self):
        ...
        frequency['background'] = torch.tensor(0.)
        frequency['background'] = frequency['background'].to(device)
        frequency['lane'] = torch.tensor(0.)
        frequency['lane'] = frequency['lane'].to(device)
        ...
        class_weights['background']=1./(torch.log(1.02+frequency['background']/(frequency['background']+frequency['lane']))+1e-12)
        class_weights['lane']=1./(torch.log(1.02+frequency['lane']/(frequency['background']+frequency['lane']))+1e-12)
    ...
    def _discriminative_loss(self):
        ...
        for i in range(num_samples):
            ...
            var_loss+=v_loss/(num_clusters+1e-12)
            dis_loss+=d_loss/(num_clusters*(num_clusters-1)+1e-12)
            reg_loss+=r_loss/(num_clusters+1e-12)
        return self.alpha*(var_loss/(num_samples+1e-12))+self.beta*(dis_loss/(num_samples+1e-12))+self.gamma*(reg_loss/(num_samples+1e-12))
