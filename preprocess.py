...
##create datasets for training from uncompressed tusimple train_data
class CreateTusimpleData():
    ...
    def __call__(self):
        if not os.path.exists('train_data/gt_binary_image'):
            os.mkdir('train_data/gt_binary_image')
        if not os.path.exists('train_data/gt_instance_image'):
            os.mkdir('train_data/gt_instance_image')
        if not os.path.exists('train_data/gt_image'):
            os.mkdir('train_data/gt_image')
        jsons=[jsonn for jsonn in os.listdir(self.tusimple) if jsonn.split('.')[-1]=='json']
        for j in jsons:
            data=[]
            ...
            for entry in data:
                ...
                cv2.imwrite(os.path.join('train_data/gt_image', '_'.join(clip.split('/')[1:])), img)
                ...
                cv2.imwrite(os.path.join('train_data/gt_binary_image', new_name), img_binary)
                cv2.imwrite(os.path.join('train_data/gt_instance_image', new_name), img_cluster)
